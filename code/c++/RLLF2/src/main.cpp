...
unique_ptr<Array4D<x1.size(), x2.size(), y1.size(), y2.size()>> my1u1(nullptr);
unique_ptr<Array4D<x1.size(), x2.size(), y1.size(), y2.size()>> my1u2(nullptr);
unique_ptr<Array4D<x1.size(), x2.size(), y1.size(), y2.size()>> my2u1(nullptr);
unique_ptr<Array4D<x1.size(), x2.size(), y1.size(), y2.size()>> my2u2(nullptr);
...
//                                               const double &fm, const double
//                                               &h, const double &m, const
//                                               double chooseM, const double
//                                               &a) noexcept {
//   return chooseM * m * (fp + fm) / 2. - a * (fp - fm) / 2.;
// }
...
__always_inline double hamiltonianInDirection(const double &fp,
  ...
  double res = 0;
  res += (1 - ww) * ((h < 1e-99) * h * fp + (h > 1e-99) * h * fm);
  res += ww * ((h - a) * fp + (h + a) * fm) / 2.;
  return res;
}

__always_inline double getMax1(const size_t &i1, const size_t &i2,
                               const size_t &i3, const size_t &i4,
                               const double &fx1, const double &fy1,
                               const double &fy2) noexcept {
  return max(0., (*mx1)[i1][i2][i3][i4] * fx1 + (*my1u1)[i1][i2][i3][i4] * fy1 +
                     (*my2u1)[i1][i2][i3][i4] * fy2);
}

__always_inline double getMax2(const size_t &i1, const size_t &i2,
                               const size_t &i3, const size_t &i4,
                               const double &fx2, const double &fy1,
                               const double &fy2) noexcept {
  return max(0., (*mx2)[i1][i2][i3][i4] * fx2 + (*my1u2)[i1][i2][i3][i4] * fy1 +
                     (*my2u2)[i1][i2][i3][i4] * fy2);
}
    ...
    for (size_t i2 = 0; i2 < x2.size(); ++i2) {
      for (size_t i3 = 0; i3 < y1.size(); ++i3) {
        for (size_t i4 = 0; i4 < y2.size(); ++i4) {
          ...
          const auto Fx1max =
              max((*Fx1p)[i1][i2][i3][i4], (*Fx1m)[i1][i2][i3][i4]);
          const auto Fx2max =
              max((*Fx2p)[i1][i2][i3][i4], (*Fx2m)[i1][i2][i3][i4]);
          const auto Fy1max =
              max((*Fy1p)[i1][i2][i3][i4], (*Fy1m)[i1][i2][i3][i4]);
          const auto Fy2max =
              max((*Fy2p)[i1][i2][i3][i4], (*Fy2m)[i1][i2][i3][i4]);
          const auto positiveMax1 =
              getMax1(i1, i2, i3, i4, Fx1max, Fy1max, Fy2max) > 0.;
          const auto positiveMax2 =
              getMax2(i1, i2, i3, i4, Fx2max, Fy1max, Fy2max) > 0.;

          const auto ax1 = abs((*hx1)[i1][i2][i3][i4] +
                               positiveMax1 * (*mx1)[i1][i2][i3][i4]);
          const auto ax2 = abs((*hx2)[i1][i2][i3][i4] +
                               positiveMax2 * (*mx2)[i1][i2][i3][i4]);
          const auto ay1 = abs((*hy1)[i1][i2][i3][i4] +
                               positiveMax1 * (*my1u1)[i1][i2][i3][i4] +
                               positiveMax2 * (*my1u2)[i1][i2][i3][i4]);
          const auto ay2 = abs((*hy2)[i1][i2][i3][i4] +
                               positiveMax1 * (*my2u1)[i1][i2][i3][i4] +
                               positiveMax2 * (*my2u2)[i1][i2][i3][i4]);

          const auto wwx1 = (*hx1)[i1][i2][i3][i4] * hx1a < 1e-99;
          hamilt +=
              hamiltonianInDirection((*Fx1p)[i1][i2][i3][i4],
                                     (*Fx1m)[i1][i2][i3][i4], wwx1, hx1a, ax1);

          const auto wwx2 = (*hx2)[i1][i2][i3][i4] * hx2a < 1e-99;
          hamilt +=
              hamiltonianInDirection((*Fx2p)[i1][i2][i3][i4],
                                     (*Fx2m)[i1][i2][i3][i4], wwx2, hx2a, ax2);

          const auto wwy1a = (*hy1)[i1][i2][i3][i4] *
                                 ((*hy1)[i1][i2][i3][i4] +
                                  chooseUMax1 * (*my1u1)[i1][i2][i3][i4]) <
                             1e-99;
          const auto wwy1b = ((*hy1)[i1][i2][i3][i4] *
                                  ((*hy1)[i1][i2][i3][i4] +
                                   chooseUMax2 * (*my1u2)[i1][i2][i3][i4]) <
                              1e-99) &&
                             !wwy1a;
          const auto wwy1c = ((*hy1)[i1][i2][i3][i4] *
                                  ((*hy1)[i1][i2][i3][i4] +
                                   chooseUMax1 * (*my1u1)[i1][i2][i3][i4] +
                                   chooseUMax2 * (*my1u2)[i1][i2][i3][i4]) <
                              1e-99) &&
          ...
          hamilt += hamiltonianInDirection((*Fy1p)[i1][i2][i3][i4],
                                           (*Fy1m)[i1][i2][i3][i4],
                                           wwy1a || wwy1b || wwy1c, hy1a, ay1);

          const auto wwy2a = (*hy2)[i1][i2][i3][i4] *
                                 ((*hy2)[i1][i2][i3][i4] +
                                  chooseUMax1 * (*my2u1)[i1][i2][i3][i4]) <
                             1e-99;
          const auto wwy2b = ((*hy2)[i1][i2][i3][i4] *
                                  ((*hy2)[i1][i2][i3][i4] +
                                   chooseUMax2 * (*my2u2)[i1][i2][i3][i4]) <
                              1e-99) &&
                             !wwy2a;
          const auto wwy2c = ((*hy2)[i1][i2][i3][i4] *
                                  ((*hy2)[i1][i2][i3][i4] +
                                   chooseUMax1 * (*my2u1)[i1][i2][i3][i4] +
                                   chooseUMax2 * (*my2u2)[i1][i2][i3][i4]) <
                              1e-99) &&
...
__always_inline void initialize() {
  ...
  my1u1 = make_unique<Array4D<x1.size(), x2.size(), y1.size(), y2.size()>>();
  my1u2 = make_unique<Array4D<x1.size(), x2.size(), y1.size(), y2.size()>>();
  my2u1 = make_unique<Array4D<x1.size(), x2.size(), y1.size(), y2.size()>>();
  my2u2 = make_unique<Array4D<x1.size(), x2.size(), y1.size(), y2.size()>>();
...
int main(int argc, char *argv[]) {
    ...
    for (size_t i2 = 0; i2 < x2.size(); ++i2) {
      for (size_t j1 = 0; j1 < y1.size(); ++j1) {
        for (size_t j2 = 0; j2 < y2.size(); ++j2) {
          ...
          (*wwy1)[i1][i2][j1][j2] =
              (*hy1)[i1][i2][j1][j2] *
                      ((*hy1)[i1][i2][j1][j2] + (*my1u1)[i1][i2][j1][j2]) +
                  (*my1u2)[i1][i2][j1][j2] <
              1e-99;
          (*ay1)[i1][i2][j1][j2] =
              max({abs((*hy1)[i1][i2][j1][j2]),
                   abs((*hy1)[i1][i2][j1][j2] + (*my1u1)[i1][i2][j1][j2]),
                   abs((*hy1)[i1][i2][j1][j2] + (*my1u2)[i1][i2][j1][j2]),
                   abs((*hy1)[i1][i2][j1][j2] + (*my1u1)[i1][i2][j1][j2] +
                       (*my1u2)[i1][i2][j1][j2])});
          (*wwy2)[i1][i2][j1][j2] =
              (*hy2)[i1][i2][j1][j2] *
                  ((*hy2)[i1][i2][j1][j2] + (*my2u1)[i1][i2][j1][j2] +
                   (*my2u2)[i1][i2][j1][j2]) <
              1e-99;
          (*ay2)[i1][i2][j1][j2] = max({
              abs((*hy2)[i1][i2][j1][j2]),
              abs((*hy2)[i1][i2][j1][j2] + (*my2u1)[i1][i2][j1][j2]),
              abs((*hy2)[i1][i2][j1][j2] + (*my2u2)[i1][i2][j1][j2]),
              abs((*hy2)[i1][i2][j1][j2] + (*my2u1)[i1][i2][j1][j2] +
                  (*my2u2)[i1][i2][j1][j2]),
          });
  ...
  // const auto dt = min(
  //     0.7 / max(mpd[0] / dx1 + mpd[1] / dx2 + mpd[2] / dy1 + mpd[3] / dy2,
  //     L),
  //     __builtin_pow(
  //         __builtin_sqrt(dx1 * dx1 + dx2 * dx2 + dy1 * dy2 + dy2 *
  //         dy2), 1.5));
  const auto dt =
      0.1 / max(mpd[0] / dx1 + mpd[1] / dx2 + mpd[2] / dy1 + mpd[3] / dy2, L);
