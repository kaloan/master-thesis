...
const auto Lexact = __builtin_sqrt(2 * (b * b + c * c + mu * mu));
...
__always_inline void calcHamiltonian() noexcept {
  ...
  for (size_t i = 0; i < x.size(); ++i) {
    for (size_t j = 0; j < y.size(); ++j) {
      ...
      const auto wwx = (*hx)[i][j] * hxa < 1e-99;
      const auto wwy = (*hy)[i][j] * hya < 1e-99;

      // Lax-Friedrichs step
      hamilt += hamiltonianInDirection((*Fxp)[i][j], (*Fxm)[i][j], wwx, hxa,
                                       abs(hxa));
      hamilt += hamiltonianInDirection((*Fyp)[i][j], (*Fym)[i][j], wwy, hya,
                                       abs(hya));
...
int main(int argc, char *argv[]) {
  ...
  mpd = {Gamma + b * (1 + k * umax), mu + c * (1 + k * umax)};
  const auto dt = 0.9 / max(mpd[0] / dx + mpd[1] / dy, L);
