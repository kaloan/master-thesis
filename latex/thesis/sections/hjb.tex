...
За намиране на ядрото на допустимост $\eqref{eq:ViabilityKernel}$ се подхожда по метода от \cite{Zidani2013}. Въвеждаме значна функция на разстоянието до границата на $\mathscr{I}$:
\begin{equation}
  \Gamma(\mathbf{z}) =
  \begin{cases}
...
\begin{equation}
\forall{t>0} \left(v(\mathbf{z}_0) = \inf_{u \in \mathscr{U}} \max\{e^{-\lambda t} v(\mathbf{z}_0), \sup_{s \in (0, t]} e^{-\lambda t} \Gamma(\mathbf{z}(s; \mathbf{z}_0; \mathbf{u}))\}\right)
\end{equation}
